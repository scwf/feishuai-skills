...
PROMPT_TEMPLATE = """你是一个科技发布会视频截图中的发布材料理解助手。

这是一张来自“{video_topic}”视频的截图。视频主题仅作为背景信息，不能据此补充图片中不可见的内容。

你的任务不是描述整张现场照片，而是优先理解截图中的发布材料内容，包括大屏幕、PPT、演示界面、图表、架构图、产品页、时间线、数据页或 Demo 界面。
...
只描述和解释图片中实际可见的信息；不要编造、补全或推断图片中没有出现的内容。
不确定或看不清时，明确写“看不清”或“无法确认”。

直接用中文自然段输出，不需要标题、字段名或代码块。
