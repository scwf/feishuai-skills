...
### 格式约束（供 email_body.py / txt_to_html.py 解析）

`email_body.py` 与 `txt_to_html.py` 依赖结构化标记解析 report。生成 report 时保持以下格式：

1. **类别头格式**：必须写 `【类别】model_vendor · 2026-06-16`，不能写 `一、模型厂商动态 (model_vendor)` 或其它变体。
2. **字段标签用中文冒号**：必须写 `事实：` `产品判断：` `推文链接：`（全角 `：`），不能写 `事实:` `产品判断:` `推文链接:`（半角 `:`）。
3. **内容换行**：字段标签和内容必须分两行——`事实：` 在一行，内容在下一行。不能写成 `事实：Moonshot 发布...`（同行）。
4. **跳转主题格式**：`【主题 NN】` 必须存在，`事实：` 必须以 `详见` 开头且含 `不重复展开`，否则会被当作有效主题。
5. **分隔线格式**：类别分隔线使用只包含 `═` 的纯行；长度不限。主题分隔线使用只包含 `─` 的纯行。
