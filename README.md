...
| `youtube-to-bilingual-video` | Orchestrates a verified YouTube-to-video workflow by composing `youtube-scraper` and `generate-and-process-subtitles`. | You need one YouTube video downloaded, Chinese-English subtitles created with Chinese above English, and a final burned-in MP4 delivered. | Adds immutable baselines, lexical-change review gates, bilingual SRT validation, bottom-position rendering, full media decode checks, and QA frames. Excludes subtitle-only work, batches, dubbing, and publishing. |
